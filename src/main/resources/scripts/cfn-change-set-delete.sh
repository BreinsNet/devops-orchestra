...
# Verify if the stack exists abd define update or delete task
...
AWS_COMMAND="aws cloudformation delete-change-set"
AWS_COMMAND="${AWS_COMMAND} --stack-name ${STACK_NAME}"
AWS_COMMAND="${AWS_COMMAND} --change-set-name ${CHANGE_SET_NAME}"

# delete a changeset
print_info "Deleting change set ${STACK_NAME} with md5sum ${MD5_SUM}"
